...
use App\Models\KelasDokumenMatkul;
...
class ProgresController extends Controller
{
    ...
    public function exportPDF($sesi)
    {
        $tempat = 'Ruang Prodi';
        $prodi_id = Auth::user()->prodi_id;

        // Ambil tahun ajaran terbaru
        $tahun_ajaran = TahunAjaran::latest('id_tahun_ajaran')->first();

        $progres_pengumpulan = KelasDokumenMatkul::whereHas('kelas.matkul', function ($query) use ($tahun_ajaran, $prodi_id) {
            $query->where('id_tahun_ajaran', $tahun_ajaran->id_tahun_ajaran)
                ->whereHas('matkul', function ($q) use ($prodi_id) {
